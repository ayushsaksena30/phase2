...
def centering(frame,target_x,target_y):
    center_x=320
    center_y=240
    if (center_x - center_tolerance <= target_x <= center_x + center_tolerance) and \
               (center_y - center_tolerance <= target_y <= center_y + center_tolerance):
                direction = "Centered"
                print("Centered")
    else:
        # Provide directional instructions based on quadrant
        direction = ""
        if target_x < center_x and target_y < center_y:
            direction = "Move Down-Right"
            set_velocity_body(gnd_speed,gnd_speed)
        elif target_x >= center_x and target_y < center_y:
